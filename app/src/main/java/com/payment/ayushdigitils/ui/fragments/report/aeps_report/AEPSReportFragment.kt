...
class AEPSReportFragment : BaseFragment(R.layout.fragment_aeps_report),
    AepsReportsAdapter.OnTrackClickListener, FiltersFragment.FiltersDialogListener {
    ...
    private val viewModel by viewModel<AepsReportsViewModel>()

    private lateinit var reportsAdapter: AepsReportsAdapter
    ...
    override fun initView() {
        observer()
        reportsAdapter = AepsReportsAdapter(this)
    ...
    private fun observer() {
        ...
        viewModel.reportItem.observe(viewLifecycleOwner, Observer {
            when (it) {
                ...
                is Resource.Error -> {
                    viewModel.showLoader.value = false
                    Timber.e(it.exception, "aeps reports ex:")
    ...
    private fun setUpReport(data: List<AEPSReportData>) {
        viewModel.showLoader.value = false
        if (data.equals("")) {
    ...
    override fun onShareClick(position: Int, reports: AEPSReportData, view: View) {
        getPermission(view)
    }

    override fun onInvoiceClick(position: Int, reports: AEPSReportData) {
        ...
        invoiceList.add(InvoiceModel("Terminal Id", reports.terminalid ?: "NA"))
        invoiceList.add(InvoiceModel("Mobile", reports.mobile ?: "NA"))
        invoiceList.add(InvoiceModel("Bcid", reports.aadhar ?: "NA"))
        invoiceList.add(InvoiceModel("Amount", getString(R.string.remitter_full_name, getString(R.string.Rs), reports.amount)))
        invoiceList.add(if (reports.aepstype.equals("AP", ignoreCase = true))
            InvoiceModel("Charge", getString(R.string.remitter_full_name, getString(R.string.Rs), reports.amount))
        else
            InvoiceModel("Commission", getString(R.string.remitter_full_name, getString(R.string.Rs), reports.amount)))
        invoiceList.add(InvoiceModel("Type", reports.aepstype ?: "NA"))
