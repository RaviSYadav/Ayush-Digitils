...
class AepsWalletReportsAdapter(
    private val listener: OnReportClickListener
) : RecyclerView.Adapter<AepsWalletReportsAdapter.AepsWalletReportsViewHolder>() {

    private val reportsList: MutableList<RechargeReportData> = mutableListOf()

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): AepsWalletReportsViewHolder {
        val itemBinding =
            ViewHolderItemReportsBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return AepsWalletReportsViewHolder(itemBinding, listener)
    }

    override fun onBindViewHolder(holder: AepsWalletReportsViewHolder, position: Int) {
    ...
    class AepsWalletReportsViewHolder(
