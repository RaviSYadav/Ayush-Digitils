...
class BillReportsAdapter(
    private val listener: OnReportClickListener
) : RecyclerView.Adapter<BillReportsAdapter.BillReportsViewHolder>() {

    private val reportsList: MutableList<RechargeReportData> = mutableListOf()

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BillReportsViewHolder {
        val itemBinding =
            ViewHolderItemReportsBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return BillReportsViewHolder(itemBinding, listener)
    }

    override fun onBindViewHolder(holder: BillReportsViewHolder, position: Int) {
