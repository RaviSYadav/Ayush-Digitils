...
class DMTReportsAdapter(
    private val listener: OnReportClickListener
) : RecyclerView.Adapter<DMTReportsAdapter.DMTReportsViewHolder>() {

    private val reportsList: MutableList<RechargeReportData> = mutableListOf()

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): DMTReportsViewHolder {
        val itemBinding =
            ViewHolderItemReportsBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return DMTReportsViewHolder(itemBinding, listener)
    }

    override fun onBindViewHolder(holder: DMTReportsViewHolder, position: Int) {
