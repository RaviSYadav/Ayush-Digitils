...
class LICReportsAdapter(
    private val listener: OnReportClickListener
) : RecyclerView.Adapter<LICReportsAdapter.LICReportsViewHolder>() {

    private val reportsList: MutableList<RechargeReportData> = mutableListOf()

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): LICReportsViewHolder {
        val itemBinding =
            ViewHolderItemReportsBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return LICReportsViewHolder(itemBinding, listener)
    }

    override fun onBindViewHolder(holder: LICReportsViewHolder, position: Int) {
