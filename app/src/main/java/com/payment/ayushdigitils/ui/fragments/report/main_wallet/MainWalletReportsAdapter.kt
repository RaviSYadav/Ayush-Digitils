...
class MainWalletReportsAdapter(
    private val listener: OnReportClickListener
) : RecyclerView.Adapter<MainWalletReportsAdapter.MainWalletReportsViewHolder>() {

    private val reportsList: MutableList<RechargeReportData> = mutableListOf()

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MainWalletReportsViewHolder {
        val itemBinding =
            ViewHolderItemReportsBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return MainWalletReportsViewHolder(itemBinding, listener)
    }

    override fun onBindViewHolder(holder: MainWalletReportsViewHolder, position: Int) {
    ...
    class MainWalletReportsViewHolder(
