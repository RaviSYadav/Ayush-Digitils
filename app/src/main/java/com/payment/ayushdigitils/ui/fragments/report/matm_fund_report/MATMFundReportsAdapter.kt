...
class MATMFundReportsAdapter(
    private val listener: OnReportClickListener
) : RecyclerView.Adapter<MATMFundReportsAdapter.MATMFundReportsViewHolder>() {

    private val reportsList: MutableList<RechargeReportData> = mutableListOf()

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MATMFundReportsViewHolder {
        val itemBinding =
            ViewHolderItemReportsBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return MATMFundReportsViewHolder(itemBinding, listener)
    }

    override fun onBindViewHolder(holder: MATMFundReportsViewHolder, position: Int) {
    ...
    class MATMFundReportsViewHolder(
