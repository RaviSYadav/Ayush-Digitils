...
class MATMReportsAdapter(
    private val listener: OnReportClickListener
) : RecyclerView.Adapter<MATMReportsAdapter.MATMReportsViewHolder>() {

    private val reportsList: MutableList<RechargeReportData> = mutableListOf()

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MATMReportsViewHolder {
        val itemBinding =
            ViewHolderItemReportsBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return MATMReportsViewHolder(itemBinding, listener)
    }

    override fun onBindViewHolder(holder: MATMReportsViewHolder, position: Int) {
