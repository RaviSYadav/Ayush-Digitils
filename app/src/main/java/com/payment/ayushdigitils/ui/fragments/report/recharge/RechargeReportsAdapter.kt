...
class RechargeReportsAdapter(
    private val listener: OnReportClickListener
) : RecyclerView.Adapter<RechargeReportsAdapter.RechargeReportsViewHolder>() {

    private val reportsList: MutableList<RechargeReportData> = mutableListOf()

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RechargeReportsViewHolder {
        val itemBinding =
            ViewHolderItemReportsBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return RechargeReportsViewHolder(itemBinding, listener)
    }

    override fun onBindViewHolder(holder: RechargeReportsViewHolder, position: Int) {
    ...
    class RechargeReportsViewHolder(
